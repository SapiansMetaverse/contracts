...
describe("OlympusStaking", () => {
    ...
    let ohmFake: FakeContract<ISAPI>;
    let sOHMFake: FakeContract<IsSAPI>;
    let gOHMFake: FakeContract<IgSAPI>;
    let distributorFake: FakeContract<IDistributor>;
    let staking: OlympusStaking;
    let authority: SapiansAuthority;
    ...
    beforeEach(async () => {
        ...
        ohmFake = await smock.fake<ISAPI>("ISAPI");
        gOHMFake = await smock.fake<IgSAPI>("IgSAPI");
        // need to be specific because IsSAPI is also defined in OLD
        sOHMFake = await smock.fake<IsSAPI>("contracts/interfaces/IsSAPI.sol:IsSAPI");
        distributorFake = await smock.fake<IDistributor>("IDistributor");
        authority = await new SapiansAuthority__factory(owner).deploy(
            governor.address,
            guardian.address,
            owner.address,
            owner.address
