...
contract SapiansTreasury is SapiansAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for SAPI
    ...
    function deposit(
        ...
        // mint SAPI needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        SAPI.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn SAPI for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        SAPI.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new SAPI using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        SAPI.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or SAPI from the treasury, using sSAPI as collateral. This might allow an
     * sSAPI holder to provide SAPI liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(SAPI)) {
            require(permissions[STATUS.SAPIDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sSAPI.changeDebt(value, msg.sender, true);
        require(sSAPI.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(SAPI)) {
            SAPI.mint(msg.sender, value);
            sapiDebt = sapiDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sSAPI.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with SAPI
     * @param _amount uint256
     */
    function repayDebtWithSAPI(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.SAPIDEBTOR][msg.sender], notApproved);
        SAPI.burnFrom(msg.sender, _amount);
        sSAPI.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        sapiDebt = sapiDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(SAPI), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SSAPI) {
            sSAPI = IsSAPI(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SSAPI) {
            // 9
            sSAPI = IsSAPI(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(SAPI.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns SAPI valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(SAPI)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return SAPI.totalSupply() - sapiDebt;
    }
}
