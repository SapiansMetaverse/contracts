...
contract sSapians is IsSAPI, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgSAPI public gSAPI; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked SAPI", "sSAPI", 9) ERC20Permit("Staked SAPI") {
    ...
    function setgSAPI(address _gSAPI) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gSAPI) == address(0), "gSAPI:  gSAPI already set");
        require(_gSAPI != address(0), "gSAPI:  gSAPI is not a valid contract");
        gSAPI = IgSAPI(_gSAPI);
    }
    ...
    /**
        @notice increases rSAPI supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sSAPI of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sSAPI
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sSAPI: insufficient balance");
    }
    ...
    // toG converts an sSAPI balance to gSAPI terms. gSAPI is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gSAPI.balanceTo(amount);
    }

    // fromG converts a gSAPI balance to sSAPI terms. sSAPI is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gSAPI.balanceFrom(amount);
    }

    // Staking contract holds excess sSAPI
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gSAPI.balanceFrom(IERC20(address(gSAPI)).totalSupply())).add(
